...
export function ReportDetail({ reporte, onBack }: ReportDetailProps) {
  ...
  const handleExportPDF = () => {
    ...
    const tableData = reporte.verificaciones.map((v) => [
      v.producto?.nombre || t("reports.unknown"),
      v.producto?.clave || "-",
      v.cantidad_sistema,
      v.cantidad_fisica,
      v.cantidad_sistema === v.cantidad_fisica
        ? t("reports.correct")
    ...
    doc.save(
      `Reporte_${reporte.id}_${new Date().toISOString().split("T")[0]}.pdf`
    );
  };
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-orange-50 p-4 rounded-lg border border-orange-200">
          <p className="text-sm text-orange-600 font-medium">
            {t("reports.withDiscrepancies")}
          </p>
          <p className="text-2xl font-bold text-orange-900">
            {reporte.total_discrepancias}
          </p>
        </div>
      </div>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {reporte.verificaciones.map((verificacion, index) => {
                ...
                return (
                  <tr
                    key={index}
                    className={isDiscrepancy ? "bg-orange-50" : ""}
                  >
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                      {verificacion.producto?.nombre || t("reports.unknown")}
                    </td>
