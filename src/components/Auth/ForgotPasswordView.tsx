...
export function ForgotPasswordView() {
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!rpcSuccess) throw new Error(t("forgotPassword.emailNotRegistered"));

      // 3. Enviar correo con EmailJS
      const emailSuccess = await sendRecoveryCode(email, code);
      if (!emailSuccess) throw new Error(t("forgotPassword.errorSendingEmail"));

      setMessage(t("forgotPassword.codeSent"));
      setStep("OTP");
    } catch (err: any) {
      console.error(err);
      setError(err.message || t("forgotPassword.errorProcessingRequest"));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    if (token.length !== 6) {
      setError(t("forgotPassword.codeDigitsError"));
      setLoading(false);
      return;
    }
  ...
  const handleUpdatePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!success) throw new Error(t("forgotPassword.invalidOrExpiredCode"));

      setMessage(t("forgotPassword.passwordUpdatedSuccess"));
      setTimeout(() => {
        navigate("/login");
      }, 2000);
    } catch (err: any) {
      console.error(err);
      setError(err.message || t("forgotPassword.errorUpdatingPassword"));
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="mb-8">
          ...
          <div className="text-right mb-4">
            ...
            <p className="text-xs text-gray-400">
              {t("forgotPassword.appDescription")}
            </p>
          </div>
        </div>
